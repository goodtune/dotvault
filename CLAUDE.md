...
- `target.delete_nulls` (default false) is accepted only for `json` and `yaml` — the two formats whose syntax has a null literal a template can render (`nullableFormats`). It is *rejected* rather than ignored on the others, because a flag that silently did nothing would leave an operator believing a retired credential had been deleted while it sat in the file untouched
