...
!!! note "Windows Group Policy"
    The `observability` block is now readable from the registry layer under `HKLM\SOFTWARE\Policies\goodtune\dotvault\Observability` (`Enabled` REG_DWORD, `Endpoint` / `Protocol` / `ExportInterval` REG_SZ, `Insecure` REG_DWORD). Author the keys directly with regedit or a hand-rolled `.reg` file pushed via Group Policy Preferences — `dotvault reg-import` does **not** currently emit an `Observability` subkey, and the ADMX policy editor doesn't surface these keys either; both are tracked as follow-ups. **Headers are intentionally not registry-managed** — they typically carry OTLP bearer tokens, so they live in `OTEL_EXPORTER_OTLP_HEADERS` (set through a machine-wide environment policy) rather than checked-in registry values.
