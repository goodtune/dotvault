...
Build: `make python-lib` compiles the bridge into the package for local pytest/editable installs; `make python-wheel` builds a platform wheel (`setup.py` rebuilds the bridge during the build, locating the Go module root by walking up for `go.mod`, so it works from a clean checkout). This is the **one** place dotvault builds with `CGO_ENABLED=1` — c-shared requires cgo — and it is a separate artefact, so the main binaries stay `CGO_ENABLED=0` pure-Go static builds. Tests run on both sides: `go test ./python/bridge/...` (handle table, category mapping, `syncEnv`) and `python -m pytest python/tests` (offline — points at a closed Vault port and asserts error categorisation). CI is `.github/workflows/python.yml` (per-OS wheels; Linux wheels are not yet manylinux-tagged — a follow-up if these are published to PyPI). The `python/` directory has its own `dependabot.yml` pip entry. See `python/README.md` for the consumer write-up.
