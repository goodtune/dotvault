...
A remote whose credentials could not be *resolved* at all — the SSH agent's sources all erred, most often a `vault-ca` mint attempted while the daemon was replacing its own Vault token — reports `offline` with an `identity` failure class, not `authentication-error`. It is a deliberate distinction: the remote refusing a credential you presented usually needs a human, where a credential source that erred is frequently recovering already, so this case retries on the ordinary backoff instead of the long floor above.
