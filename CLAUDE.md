...
Scope is the **read-only + cached-auth** subset of the facade plus the socket-forwarded **peer actions**: `AuthenticateCached` (never prompts), `IdentityName`, `Token`, `ReadKVField`, `ReadUserSecret`, `browse`, and `notify`. Interactive `Login`/`Authenticate` (OIDC browser, LDAP terminal) are deliberately excluded — driving them across an FFI boundary from inside a Python process is awkward and not what a library caller wants; such callers provision a token out of band (`dotvault login`, or the daemon) and the bindings consume it. `browse`/`notify` are in scope precisely because they need **no** local token — they post over the peer socket — so a headless Python program can surface a URL or a notification on the workstation.
