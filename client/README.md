...
| `AuthenticateCached(ctx)` | env → token file (skipped under `mtls+os`) → local API socket → peer socket borrow (if `TokenSockets` is set) → this host's client certificate (if the auth method is `mtls*`). Never prompts. `ErrLoginRequired` if no usable token. | Side-effect-free preflight (`doctor`), non-interactive / CI callers. |
...
If either socket field is set, `AuthenticateCached` borrows a live token from a peer after `DOTVAULT_TOKEN` and the token file come up empty, before reporting `ErrLoginRequired`. Two tiers are consulted, in order: `VaultConfig.APISocket` (dotvault's `api` section — the *local* daemon's API socket) first, then `VaultConfig.TokenSockets` (`vault.token_socket` — peer daemons' sockets, typically SSH `RemoteForward`s). The local socket is preferred because it is the more stable of the two: the forwarded one dies with the SSH session, so a long-running consumer that borrowed only from it would fail its next read once the session ended. `LoadConfig` fills both from the operator's config, so a consumer inherits the ordering without doing anything. The borrow is a plain HTTP GET over the socket with no browser or prompt, so it stays within the cached, side-effect-free contract — a consumer on a host with no local token but a live peer socket (the SSH `RemoteForward` topology) reads secrets without an interactive login of its own. It is best-effort: a missing or stale socket simply yields no token.

`TokenSockets` holds **patterns**, not just paths: a literal path, or a final-segment glob such as `~/.ssh/dotvault.*.sock`, which is how one config serves a host that several workstations forward into. Within that tier the most recently seen live peer is borrowed from first — a freshly reconnected laptop is the one actually there — and missing or stale sockets are skipped.

## Certificate auth: consumption only
...
**`Login` refuses under certificate auth**, returning `ErrLoginRequired`. A fresh login under a cert method would, on a host with no usable credential, run the bootstrap: an OIDC browser flow or an LDAP terminal prompt, then mint a certificate and write it to this host. None of that belongs in a library call. Enrolment stays with the dotvault daemon or CLI, which own the host; the facade consumes what they provisioned. Concretely, the facade can **use** a certificate but can never mint, rotate, or bootstrap one — `VaultConfig.MTLS` carries only the cert mount, role, and storage directory, with no PKI mount/role, bootstrap method, or BYO paths.

## Peer actions: Browse, Notify, and Clipboard

Over the same `TokenSockets` peers, the client can ask the workstation dotvault to **open a URL in a browser**, **raise a desktop notification**, or **put text on the clipboard** — the programmatic equivalents of `dotvault browse`/`dotvault notify`/`dotvault clipboard`. This is for the headless-consumer topology: a program on a machine with no browser hands a URL, a notification, or a value to paste back over the forwarded socket, so a browser-driven flow (an OAuth page, a report link), a "job finished" toast, or the one-time token that page asks for lands on the workstation where a human is looking.

Unlike the borrow, which takes one token, the peer actions **post to every live peer** and succeed if any accepts: with several workstations forwarded into the host there is no way to know which one the user is sitting at, so the URL or the value goes to all of them. `APISocket` is deliberately excluded — routing a browse to the local daemon would open a browser on the headless host nobody is looking at. A peer that rejects the request as invalid (a 4xx — bad URL, unknown level) surfaces as a plain error carrying its message even if another peer accepted, because bad input is bad everywhere.
