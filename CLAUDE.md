...
**Borrow order is local-first.** `config.TokenBorrowSockets()` returns `[api socket, vault.token_socket]` — most-stable-first, since the forwarded peer is the one that disappears. The seam is `auth.FetchTokenFromSockets(ctx, paths) (token, source)`; the carriers are `auth.Manager.TokenSockets`, `LifecycleManager.SetTokenSockets`, and `client.VaultConfig.APISocket` + `TokenSocket` (the facade keeps both as exported strings and orders them in `borrowSockets()`). The daemon excludes its **own** socket (`daemonBorrowSockets`, `cmd/dotvault/apisocket.go`) — borrowing from itself would either return the token it already holds, muddying the log line that says where a token came from, or 401 against its own listener before it has authenticated. **Direction matters and the orders differ:** the peer actions (`browse`/`notify`/`clipboard`) deliberately keep using `vault.token_socket` alone, because their purpose is to reach the workstation where a human is looking — routing them to the local daemon would open a browser on the headless host nobody is sitting at.
