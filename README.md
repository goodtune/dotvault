...
- **Remote configuration** — Optionally overlay the local config with a partial document served by the companion `dotvault-config` service, which composes per-OS, per-group (static or LDAP), and per-user layers so a fleet operator delivers personalised rules and enrolments without touching any machine's base config
