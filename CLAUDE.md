...
`cmd/dotvault-config` is the server side of the `remote_config` overlay (designs: `docs/superpowers/specs/2026-06-10-remote-config-design.md` and `2026-06-12-multi-dimension-composition.md`; user docs: `docs/services/dotvault-config.md`). It composes layer documents — `config.Partial`s stored under canonical keys — folding with `config.MergePartial` so the service composes exactly the way clients merge. Layers are addressable by any combination of four dimensions (`os` ← `X-Dotvault-OS` lowercased, `group` ← resolver, multi-valued, `device` ← `X-Dotvault-Hostname` lowercased/optional, `user` ← `X-Dotvault-User`); a key is `global` or `kind/value[/value…]` where the kind is the `+`-joined combination in canonical spelling (`os+group`, never `group+os` — rejected naming the canonical form). The service config's `composition.order` is an explicit ordered kind list: the declared order IS the precedence (no implicit specificity), an unlisted kind is never looked up or served (admin layer PUT answers 422 and seed refuses, so dead config fails at publish), and omitting the block keeps the default `[global, os, group, user]` — byte-identical to the original fixed sequence, so existing deployments and their ETags are unaffected. A kind containing `group` expands once per sorted group at its position; a kind referencing a dimension the request didn't supply skips. The composed YAML is deterministic (yaml.v3 sorts map keys) so the `ETag` (quoted sha256 of the bytes) is stable and an unchanged document costs a `304`. Missing layers skip silently; a present-but-invalid layer is a 500 naming the layer key (`configsvc.LayerError`), never a silently dropped layer.

Cobra commands: `serve` (HTTP service; `--seed <dir>` dev convenience), `seed` (publish a layer directory — `global.yaml`, optional `groups.yaml` static membership, and one directory per kind with one nested level per dimension value, e.g. `os/linux.yaml`, `os+group/linux/sydney.yaml` — validating everything via `ParsePartial` + `Validate` + the composition-order gate before any write; stray files, unknown directories, and wrong-depth layer files are errors), `compose` (offline composition for debugging; `--groups` bypasses the resolver, `--device` feeds the optional device dimension), `version`. The service config (`--config`, see `configsvc.dev.yaml`) is strict-parsed: unknown keys are a hard error.
