...
The built assets are embedded into the binary via `embed.FS`. **The bundle is a committed artefact** — `internal/web/static/app.js` is checked in, and the `Frontend bundle freshness` job in `.github/workflows/test.yml` fails the build if it is stale relative to `frontend/src` or `package-lock.json`. Rebuild and `git add internal/web/static/app.js` in the same commit as any frontend source change.
