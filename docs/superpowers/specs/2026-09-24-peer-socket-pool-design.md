...
`login`, `status`, `sync`, and `enrol` build a transient pool (resolve, no watcher) from the same exclusion helpers they use today. `dotvault status` reports `source: borrowed from peer socket <path>` with the member path the pool answered from, not the pattern. `browse`, `notify`, and `clipboard` build their pool from `config.PeerActionSockets()` and call `Broadcast`; any error still degrades to the local action exactly as today, logged at debug.

### `client/` facade

`borrowSockets()` feeds a transient pool for `Authenticate` and `AuthenticateCached`; `peerAction` calls `Broadcast` over `TokenSockets`. The error mapping is unchanged: `ErrNoPeers` and transport failure → `ErrPeerUnavailable`; a 4xx → a plain error carrying the peer's message. The Python bridge needs no change.
...
- The `remote_socket` in the status block and in `dotvault ssh list` keeps reporting the expanded path, as it already does for `~/`; the web edit form and `ssh.yaml` show the template.
...
1. `GET /api/v1/status` → `version`. Skip, at debug, unless the value parses as a semantic version at or above the release that introduces `{{HOSTNAME}}` expansion (a named constant, `remoteSocketTemplateSince`); an empty or unparseable value (a dev build) is treated as new. This is the guard that keeps an old workstation from being handed a template it cannot expand and binding a literal-braced socket.
2. `GET /api/v1/ssh/remotes` → the literal `ssh.yaml` entries. Candidates are entries whose stored `remote_socket` is **exactly** `~/.ssh/dotvault.sock`. An explicit absolute spelling of the same path is left alone: that is an operator's choice, not the untouched default.
3. Self-identification of each candidate's `host`: case-insensitive equality with `os.Hostname()` or its first label; failing that, `net.LookupHost(host)` under a 2-second bound yields an address present in `net.InterfaceAddrs()`. Loopback addresses never match. Every matching candidate is migrated; aliases for the same box are distinct entries and each gets its own PATCH.
4. `GET /api/v1/csrf`, then `PATCH /api/v1/ssh/remotes/<host>` with body `{"remote_socket": "~/.ssh/dotvault.{{HOSTNAME}}.sock"}` and the `X-CSRF-Token` header — `dotvault ssh edit --socket`'s request, verbatim. The workstation's `Registry.Patch` saves and reconciles, which rebinds the forward at the new path. That tears down the connection carrying the PATCH response, so a transport error after the request has been written is logged at INFO as "probably applied"; the new socket appearing via inotify is the confirmation, and the vanished old one drops out of the pool naturally. Any other failure is a single WARN naming the host.
...
A GitHub issue, "Remove pre-1.0 token_socket compatibility", tracks the removal of: the migration itself, the scalar `token_socket` form, the `Vault\TokenSocket` REG_SZ fallback, and `~/.ssh/dotvault.sock` in the default pattern list. Each code site references the issue number.
