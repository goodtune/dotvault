...
One behaviour change for existing deployments: a keyless rule is now synced **before the daemon authenticates**, as soon as `dotvault run` has its listeners up — so the files these rules manage are written on a host that has not yet obtained a Vault token, where previously they waited for one. It needs no token, so it does not wait for one, and that matters most on exactly the host in the example above: the `RemoteForward` the rule writes is what creates the socket the daemon then borrows its token over, and a daemon that idles waiting for a token (headless with nothing to borrow yet, `borrow_only` with the forward not up, or web mode with nobody at the browser) would otherwise never write it. The same pass runs ahead of the first-run enrolment wizard, which blocks the first full sync even on a host that authenticated normally.

Nothing else about the rule changes: the file is written with the same 0600 permissions and the same merge, the rules are synced again in the first full cycle (where the usual skip logic makes that a no-op unless something changed), and a rule that fails is logged and skipped rather than holding up startup. `dotvault sync` is unaffected — the one-shot command still authenticates first and syncs every rule together.
