...
A copy enrolment plays no part in the web UI's [first-run wizard](../web-ui.md#first-run-enrolment-wizard) and is not shown there. It cannot raise the wizard — there would be nothing in it for you to do — and, importantly, its completing does not suppress it either: because a copy enrolment finishes on its own, often before you have seen anything, counting it as progress would have hidden the wizard from a brand-new user who still had real enrolments waiting. It appears under Enrolments on the main site like any other, where you can inspect it and re-run it on demand.
