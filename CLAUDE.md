...
A per-user overlay at `paths.UserConfigPath()` merges over whichever of those the daemon loaded, but it may carry only the `fuse` section and each field ratchets rather than overwriting — see "Per-user config overlay". HKCU staying unread is not in tension with that: the overlay is a narrow, one-way-ratcheted preference file, not a second policy source.
