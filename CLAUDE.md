...
When HKLM registry keys exist at `SOFTWARE\Policies\goodtune\dotvault`, the daemon loads all config from registry and ignores the YAML file. The `registryLayer` struct reads Vault, Sync, Web, Agent (scalar transport: `Enabled`, `UnixPath`, `WindowsPipe`, and the tri-state `WindowsPutty` REG_DWORD — absent leaves the `*bool` nil so the default-true applies), and Observability settings from typed subkeys (REG_SZ, REG_DWORD). Rules are subkeys under `Rules\{RuleName}` with an optional `OAuth` subkey. Enrolments are subkeys under `Enrolments\{Name}` with an optional `Settings` subkey.
