...
The unit tests in `internal/dockervol` post JSON at the handler and drive the driver directly: option parsing, materialisation (layouts, selection, pruning, atomic replacement, the symlink escape, modes), the lifecycle (reference counting, persistence and resume, the Mount/Unmount and Remove races, concurrent first mounts), and the refresh policy against a fake event source. What they cannot show is that an engine agrees with the driver about the protocol, so `test/integration/podman_test.go` (`make podman-e2e`; the `podman-e2e` job in CI on `ubuntu-24.04`; opt-in via `DOTVAULT_PODMAN_E2E=1` because it pulls images and runs containers) has a rootless Podman create a volume through the real socket, mount it into a container, observe a rewrite in Vault land in place while a container holds the volume, read `dotvault status` over the same socket, watch the directory go at the last unmount, and exercise `mode=0444`, `layout=fields` and the unknown-option refusal — against a dev-mode Vault running under the same podman. Rootless Docker is not driven in CI: the runner's dockerd is rootful, and a rootful engine reaching a per-user socket is the arrangement the guide tells operators to think twice about.
