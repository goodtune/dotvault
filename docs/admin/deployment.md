...
- **A queued connection is bounded by startup, not by authentication.** Both surfaces begin serving before the daemon has a Vault token: the API socket answers an honest `401`, and the SSH agent answers "no identities" (and refuses a signature) until one arrives. (The [Docker volume plugin](../guide/docker-volumes.md), which is not socket-activated, starts pre-auth in the same spirit and refuses a `Mount` that would need Vault with a message naming the cause; its socket and volumes live under the same `RuntimeDirectory=dotvault`, and `RuntimeDirectoryPreserve=yes` is what keeps a running container's volume alive across a daemon restart.) That distinction matters because the backlog has no timeout of its own — a daemon that can never obtain a token (no local token, no peer to borrow from) would otherwise leave every client blocked forever on a connection systemd had already accepted.
