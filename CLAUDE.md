...
**Identity is the OS user, not the token.** `IdentityName` (deliberately ctx-less — it does a local OS lookup, no Vault call, so the consumer's sketched `IdentityName(ctx)` shed its context) returns `paths.Username()` (the OS account, `DOMAIN\` prefix stripped) — the same value the sync engine and enrolment manager use to lay out `kv/users/<user>/...`. It is deliberately *not* derived from the Vault token's `display_name`/entity/metadata, because that is not what dotvault writes under. Consumers must therefore run as the same OS user as the dotvault that populated the secrets (normally true for a per-user daemon). This is the one place the public API contradicts a naive "identity comes from the token" assumption, and it is load-bearing — changing it would be a path-layout migration, not a facade tweak.
