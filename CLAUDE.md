...
`internal/vaultfs/` mounts the user's own KV subtree as a filesystem: a secret becomes a file whose contents are its data section rendered as JSON (`jq . ~/.dotvault/gh`), a KV folder becomes a directory, and `stat` reports the current version's `created_time` as the file's mtime. Disabled by default (`fuse.enabled`); the daemon mounts after its first successful Vault auth and unmounts on shutdown. It is the read-anything counterpart to sync rules, which write specific fields into specific files on a schedule.
...
**A KV path must be either a secret or a folder, never both.** Vault permits both (`users/you/databricks` alongside `users/you/databricks/prod`, which LIST returns as `databricks` and `databricks/`); a filesystem name cannot be. The overlap is unsupported and belongs in the Vault policy/layout, not in code: the behaviour is deterministic — the directory wins, so grouped enrolment keys stay reachable — and the daemon warns once per path, but the shadowed secret has no path in the mount.
