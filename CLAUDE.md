...
So: for *how* something works, read the package — `internal/vaulttest`, `internal/uds`, `internal/vaultfs`, `internal/securestore`, and `internal/auth/method.go` are the models for the density expected. For user-facing behaviour, `docs/` is the published reference (`docs/authentication/`, `docs/configuration/`, `docs/guide/`, `docs/services/`).
